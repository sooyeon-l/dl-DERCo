...
def make_subject_ablation_summary(df: pd.DataFrame) -> pd.DataFrame:
    ...
    summary = (
        ...
        .agg(
            ...
            std_auc=("oof_auc", "std"),
            sem_auc=("oof_auc", lambda x: x.std(ddof=1) / np.sqrt(len(x))),
            mean_bal_acc=("oof_balanced_accuracy", "mean"),
            std_bal_acc=("oof_balanced_accuracy", "std"),
            sem_bal_acc=("oof_balanced_accuracy", lambda x: x.std(ddof=1) / np.sqrt(len(x))),
